...
if isfield(loadedData, 'buildingsLayer') && isa(loadedData.buildingsLayer, 'table')
    ...
    if ismember(targetColumnName, loadedData.buildingsLayer.Properties.VariableNames)
        ...
        % Initialize matrix for optimal UAV parameters (power, height, radius, users served, bandwidth)
        optimal_data = zeros(numClusters, 5);
        ...
        for i = 1:numClusters
            ...
            for j = 1:clusterBuildingCounts(i)
                ...
                optimal_data(i, :) = [power_threshold, height_threshold, coverage_radius, num_users_served, bw_uav];
            end
        end
        ...
        disp('---------------------------------------');
        disp('Cluster | Power (W) | Height (m) | Radius (m) | Users Served | Bandwidth (MHz)');
        disp('---------------------------------------');
        for i = 1:numClusters
