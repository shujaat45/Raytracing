    ...
    if ismember(targetColumnName, loadedData.buildingsLayer.Properties.VariableNames)
        ...
        for i = 1:numClusters
            ...
            if ~isempty(clusterLatitudes)
                ...
                coverageArea(i) = calculateCoverageArea(clusterLatitudes, clusterLongitudes);  % Using channel model
            end
        end
...
% Add the calculateCoverageArea function at the end of the script
function coverageArea = calculateCoverageArea(clusterLatitudes, clusterLongitudes)
    % Simulate a simple channel model for coverage area calculation in mountainous areas
    % This is a placeholder for a more sophisticated model based on terrain characteristics,
    % signal propagation properties, and UAV capabilities.
    coverageArea = 100;  % Placeholder value (in square kilometers) for demonstration purposes
end
